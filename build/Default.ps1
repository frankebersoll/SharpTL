Properties {
    ...
    $solution_path = "$code_dir\SharpTL\SharpTL.sln"
    $assembly_info_path = "$code_dir\SharpTL\CommonAssemblyInfo.cs"
}
...
Task Build -depends ValidateConfig -description "Builds outdated artifacts." {	
	Write-Host "Building SharpTL.sln" -ForegroundColor Green
	Exec { msbuild "$code_dir\SharpTL\SharpTL.sln" /t:Build /p:Configuration=$config /v:quiet } 
}

Task Clean -depends ValidateConfig -description "Deletes all build artifacts." {
	Write-Host "Cleaning SharpTL.sln" -ForegroundColor Green
...
Task Pack -depends Build -description "Packs to a NuGet package." {
    ...
    Exec { nuget pack "$code_dir\SharpTL\SharpTL.nuspec" -Properties "config=$config" -Symbols -Version "$version" -OutputDirectory "$packages_dir" }
}
